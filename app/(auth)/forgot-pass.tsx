...
export default function ForgotPasswordScreen() {
  ...
  const handlePasswordReset = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'No se pudo enviar el correo. Verifica la dirección ingresada.');
    }
  };
  ...
  return (
    <View style={{ flex: 1, justifyContent: 'center', padding: 20, backgroundColor: '#fff' }}>
      ...
      <View style={{ marginBottom: 25 }}>
        <Text style={{ marginBottom: 8, fontWeight: '500' }}>Correo Electrónico</Text>
        <TextInput
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
        <TouchableOpacity onPress={() => router.push('/login')}>
          <Text style={{ color: '#007AFF', fontWeight: 'bold' }}>Volver al Inicio de Sesión</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
